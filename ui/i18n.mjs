...
const STRINGS = {
  'zh-CN': {
    ...
    setup_saving: '保存中…',
    connect_h1: '连接行程服务',
    connect_lead: '旅行团把每趟行程存在一个你自己运行的行程服务里。本机 Docker 里已经在跑的会被自动识别，不用登录。没有的话，一键运行一个。',
    connect_h1_login: '行程服务在，缺一个登录',
    connect_lead_login: '{url} 有回应，但这个 app 还没有它的管理员登录，也没能从本机 Docker 里读到。到设置里填一次就好。',
    connect_run: '帮我运行（Docker）',
    connect_run_note: '一键启动。管理员账号自动生成，密码只存在本机的 trek.env 里。',
    connect_settings: '连接我已经在跑的',
    connect_settings_note: '地址和登录都在设置里，以后随时改。',
    connect_enter_login: '去设置里填登录',
    connect_login_note: '填的是那个行程服务的管理员邮箱和密码。只存在本机，权限 600。',
    run_custom: '自定义账号和端口',
    run_created: '行程服务已启动。管理员账号 {email}，密码在 {path}。',
    ...
    svc_login_ticket: '沿用之前的登录票据；到期前请在这里填登录',
    svc_login_ticket_only: '只有票据，没有登录',
  ...
  en: {
    ...
    setup_have_title: 'I already run one',
    ...
    setup_save: 'Save',
    setup_saving: 'Saving…',
    connect_h1: 'Connect a trip planner',
    connect_lead: 'The crew keeps every trip in a trip planner you run yourself. One already running in Docker on this machine is picked up on its own, no login asked. Without one, start one with a click.',
    connect_h1_login: 'The planner is up; it needs a login',
    connect_lead_login: '{url} answers, but this app has no admin login for it and could not read one from local Docker. Enter it once in Settings.',
    connect_run: 'Run it for me (Docker)',
    connect_run_note: 'One click. The admin login is generated for you; the password stays in trek.env on this machine.',
    connect_settings: 'Connect one I already run',
    connect_settings_note: 'Address and login live in Settings, changeable any time.',
    connect_enter_login: 'Enter the login in Settings',
    connect_login_note: 'That planner\'s admin email and password. Stored on this machine only, mode 600.',
    run_custom: 'Custom login and port',
    run_created: 'Trip planner started. Admin login {email}; the password is in {path}.',
    ...
    svc_login_ticket: 'Reusing an earlier login ticket; enter the login here before it expires',
    svc_login_ticket_only: 'Ticket only, no login',
